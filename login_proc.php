...
// ----- konfigurasi debug -----
$DEBUG = false; // ubah ke false setelah debugging selesai
...
if (!$ldapbind) {
	...
	if (!isset($link) || !($link instanceof mysqli)) {
		...
		if ($DEBUG) {
			die("No DB connection available to validate user. Check koneksi.php");
		}
	...
	if ($rows > 0) {
		...
		if ($db_pass === null) {
			...
			if ($DEBUG) {
				echo "User record found, but no password column.";
				exit;
			}
			header("Location: login.php?error=serv");
			exit;
